...
keys = [
    ...
    Key("S-A-c", lazy.window.kill(), desc="Kill focused window"),
    ...
    Key("C-A-f", lazy.spawn("float"), desc="Set floating"),
...
groups = [
    Group("1", layout="tile",       matches=[Match(wm_class=["thunar", "Subl", "vivaldi-stable", "Alacritty", ])]),
    Group("2", layout="max",        matches=[Match(wm_class=["emacs", "geany", "TelegramDesktop"])]),
    Group("3", layout="monadtall",  matches=[Match(wm_class=["inkscape", "Brave-browser", "ristretto", "nitrogen"])]),
    Group("4", layout="monadtall",  matches=[Match(wm_class=["qpdfview", "", "nemo", "caja", "pcmanfm"])]),
    Group("5", layout="max",        matches=[Match(wm_class=["telegramDesktop"])]),
...
floating_layout = layout.Floating(float_rules=[
    ...
    *layout.Floating.default_float_rules,
    ...
    Match(title='Media viewer'),  # gitk
    Match(title='pinentry'),  # GPG key password entry
    ],
    border_focus=colors[12] [0]
)
auto_fullscreen = True
focus_on_window_activation = "small"
